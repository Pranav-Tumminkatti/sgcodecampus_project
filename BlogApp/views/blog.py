...
def display_post(id):
    ...
    if request.method == 'POST':
        new = Comment(blog_id=int(id), profile_id=current_user.id, comment=form.comment.data, time = datetime.now())
    ...
    if post_data: 
        ...
        if (post_data.author.id == current_user.id) or (current_user.id == 1):
            is_owner=True
        else:
            is_owner=False
...
def create_post():    
    ...
    if request.method  == 'POST' and form.validate_on_submit():
        ...
        try:
            ...
            synop = "Error 404. Something went terribly wrong. Please edit this post and enter your own summary. We sincerely apologise for the inconvenience caused!"
...
def edit_post(id):
    ...
    if post_data is None:
        ...
        return redirect(url_for('general.your_blogs'))
    elif (post_data.author.id == current_user.id) or (current_user.id == 1):
        tgs = []
        tags = post_data.tags
        for item in tags:
...
def delete_post(id):                                
    ...
    if (post_data is not None) and ((post_data.author.id == current_user.id) or (current_user.id == 1)):
        db.session.delete(post_data)
        db.session.commit()  
        flash('Blog successfully deleted!', 'green')
    else:
        flash("Error! Please check your URL again!", 'red')
...
def edit_c(id):
    ...
    if (comment.profile_id != current_user.id) and (current_user.id != 1):
        flash('You cannot edit this comment!', 'red')
        return redirect('/display_post/' + str(blog_id))
    else:
        if request.method == 'POST' and form.validate_on_submit():
            print('updated')
            ...
            db.session.commit()
            return redirect('/display_post/' + str(blog_id) + '#ld')
        return render_template('blog/edit_comment.html',form=form,id=id)
...
def delete_comment(id):
    ...
    comment = Comment.query.filter_by(id=id).first()
    blog_id = comment.blog.id
    if (comment.profile_id == current_user.id) or (current_user.id == 1):
        db.session.delete(comment)
        db.session.commit()
        flash('Comment successfully deleted!', 'green')   
    else:
        flash('You cannot delete someone else\'s comment. Don\'t do that. It doesn\'t look good on you.','red')
        return redirect('/display_post/'+str(blog_id))
